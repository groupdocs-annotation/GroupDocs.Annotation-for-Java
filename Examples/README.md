...
GroupDocs.Annotation for Java is a powerful API that enables developers to add annotation capabilities to their Java applications seamlessly. With extensive markup tools, users can highlight, strikeout, comment on text and images, and control the appearance of annotations with numerous properties. This [Examples](https://github.com/groupdocs-annotation/GroupDocs.Annotation-for-Java/tree/master/Examples) will help you get started with integrating annotation features into your Java applications.
...
2. Clone (using command `git clone https://github.com/groupdocs-annotation/GroupDocs.Annotation-for-Java.git`) or download the repository from GitHub.
3. Open the project in your favorite IDE (e.g., IntelliJ IDEA, Eclipse, NetBeans).

#### Configuring Files and Output Directories

The `FilesUtils` class provides utility methods to handle file paths. You can configure these paths using system properties or environment variables:

Setting up input file directory path: `FilesUtils` will look for sample inputs in this location. By default, it is `Resources/SampleFiles`.

* System property `sample.files.path`
* Environment variable `FILES_PATH`
* Environment variable `GROUPDOCS_FILES_PATH`

Setting up output file directory path: This location will store result files generated by samples. By default, it is `Resources/Output`.

* System property `sample.output.path`
* Environment variable `OUTPUT_PATH`
* Environment variable `GROUPDOCS_OUTPUT_PATH`
...
3. Locate the `com.groupdocs.examples.annotation` package and navigate to its contents.
...
3. Locate the `com.groupdocs.examples.annotation` package in `src/test/java` directory and navigate to its contents.
...
The `com.groupdocs.examples.annotation.utils.LicenseUtils#createLicenseStream()` method creates a license input stream by searching for a valid license file from various predefined locations.
